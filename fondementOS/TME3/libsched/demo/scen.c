...
int SJFElect(void) {
  ...
  {
    if (Tproc[i].duration < duration_min && Tproc[i].flag == RUN)
    {
...
int ApproxSJF(void) {
  ...
  {
    if (Tproc[i].ncpu < cpu_min && Tproc[i].flag == RUN)
    {
      cpu_min = Tproc[i].duration;
      p = Tproc[i].pid;
...
int main (int argc, char *argv[]) {
  ...
  // // Definir une nouvelle primitive d'election SJF
  // SchedParam(NEW, 0.3, SJFElect);
  
  // Definir une nouvelle primitive d'election ASJF sans quantum
  // SchedParam(NEW, 0., ApproxSJF);
  
  // Definir une nouvelle primitive d'election ASJF
