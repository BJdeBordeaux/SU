...
ARBRES ORDONNÉS
 • •
create table Arbre( id number(10),
par number(10),
pos number(2),
constraint pk primary key (id),
constraint fk foreign key (par) references Arbre (id));
id est l'identifiant du nœud, par est l'identifiant du parent,
  1
2I009 s
 • pos est la position parmi les enfants ;
1. Inserez l'arbre binaire (1(2(4,5),3(6,7(8,9))))) dans la base de données.
2. Est-ce qu'on peut insérer des données incohérentes ?
3. Comment faut il modifier le schéma pour empêcher cette incohérence.
4. Effacez le sous-arbre (2(4,5)) de la base de données.
5. Effacez le sous-arbre 3(6,7(8,9)) de la base de données.
6. Est-ce que l'instruction suivante est possible sur l'arbre initial (avant l'effacement) ?
delete from Arbre where id=3;
7. Comment peut-on modifier le schéma pour effacer le sous-arbre avec l'instruction précédente ?
--8. Est-ce qu'il est possible de modifier l'identifiant d'un nœud ? Par exemple : update Arbre set
id=10 where id = 3;
--9. Comment peut-on modifier le schéma pour permettre l'instruction précédente ? 10. Quel est le résultat des instructions suivantes (sur l'arbre initial):
